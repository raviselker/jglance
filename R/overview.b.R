...
overviewClass <- if (requireNamespace('jmvcore', quietly=TRUE)) R6::R6Class(
    "overviewClass",
        ...
        .run = function() {
            ...
            variables <- lapply(varNames, function(name) {
                ...
                isId <- isTRUE(attr(v, 'jmv-id')) ||
                        (!is.null(measureType) && tolower(as.character(measureType)) == 'id')
                ...
                if (isId) {
                    ...
                    levels_list <- lapply(displayedNames, function(lvl) {
                        list(
                            name  = lvl,
                            count = as.integer(if (lvl %in% names(tbl)) tbl[[lvl]] else 0)
                        )
                    })
                    list(
                        name        = name,
                        ...
                        nTruncated  = as.integer(max(0, nLevels - length(displayedNames))),
                        levels      = levels_list
                    )
                    ...
                    if (n < 2) {
                        list(
                            ...
                            histogram   = list(edges = list(), counts = list())
                        )
                    } else {
                        h <- hist(vClean, plot = FALSE, breaks = 12)
                        qs <- as.numeric(quantile(vClean, c(0.25, 0.75), na.rm = TRUE))
                    ...
                    levels_list <- lapply(seq_along(displayed), function(i) {
                        list(
                    ...
                    list(
                        ...
                        nMissing    = as.integer(nMissing),
                        nLevels     = as.integer(nLevels),
                        nTruncated  = as.integer(max(0, nLevels - length(displayed))),
                        levels      = levels_list
                    )
                }
            })
...
(function render() {
    ...
    var data = ', json, ';
    Jglance.createOverview("#', id, '", data);
})();
</script>
'))
        }
