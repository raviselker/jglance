...
import { useOverviewState, OVERVIEW_STATE_KEY } from './composables/useOverviewState'
...
const filteredVars = computed(() => {
    ...
    return props.data.variables.filter(v => {
        ...
        if (q) {
            ...
            const inDesc = !!v.description && v.description.toLowerCase().includes(q)
...
const summaryStats = computed(() => {
    ...
    let nContinuous = 0, nNominal = 0, nOrdinal = 0, nId = 0
    let totalMissing = 0
    for (const v of all) {
        switch (v.type) {
            case 'continuous': nContinuous++; break
            case 'nominal':    nNominal++;    break
            case 'ordinal':    nOrdinal++;    break
            case 'id':         nId++;         break
        }
        totalMissing += v.nMissing
    }
    return {
        nContinuous, nNominal, nOrdinal, nId,
...
<template>
    <article class="jglance jglance--overview">
        ...
        <div v-if="data.variables.length === 0" class="jglance__empty">
            ...
            <p class="jglance__empty-hint">Drop variables into the supplier on the left to see an overview.</p>
        </div>
