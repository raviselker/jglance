...
const itemsWithHeaders = computed<Item[]>(() => {
    ...
    if (state.sortMode.value !== 'type' || vars.length === 0) {
        return vars.map(v => ({ kind: 'row' as const, key: v.name, variable: v }))
    }
    ...
    for (const v of vars) {
        if (v.type !== currentType) {
            if (headerIndex >= 0) {
                (result[headerIndex] as { kind: 'header'; count: number }).count = groupCount
            }
            currentType = v.type
            groupCount = 0
            result.push({
...
function onRowKey(event: KeyboardEvent) {
    ...
    if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
        ...
        const buttons = listRef.value?.querySelectorAll<HTMLButtonElement>('.row')
        if (!buttons) return
...
function typeLabel(t: IVariableSummary['type']): string {
    return t === 'continuous' ? 'continuous'
         : t === 'ordinal' ? 'ordinal'
         : t === 'id' ? 'id'
         : 'nominal'
}
...
function isContinuous(v: IVariableSummary): v is import('../../common').IContinuousSummary {
...
function previewStat(v: IVariableSummary): string {
    ...
    if (v.levels.length === 0) return `${v.nLevels} level${v.nLevels === 1 ? '' : 's'}`
    const top = v.levels.reduce((best, l) => l.count > best.count ? l : best, v.levels[0]!)
...
<template>
    <ul ref="listRef" class="list" v-if="variables.length > 0">
        <template v-for="item in itemsWithHeaders" :key="item.key">
            <li v-if="item.kind === 'header'" class="list__group" :aria-hidden="true">
                <span class="list__group-label">{{ item.label }}</span>
                <span class="list__group-count">{{ item.count }} {{ item.count === 1 ? 'var' : 'vars' }}</span>
            </li>
            <li v-else
                class="list__item"
            ...
            >
                ...
                >
                    <span class="row__type-dot" :class="`is-${item.variable.type}`" aria-hidden="true"></span>
                    <span class="row__name">{{ item.variable.name }}</span>
                    <span class="row__type">{{ typeLabel(item.variable.type) }}</span>
                    <span class="row__spark">
                        ...
                        <span class="row__preview" :aria-hidden="true">
                            <Sparkline :variable="item.variable" :width="220" :height="60" />
                            <span class="row__preview-stat">{{ previewStat(item.variable) }}</span>
                        </span>
                    </span>
                    <span class="row__missing" :class="{ 'is-warn': item.variable.nMissing > 0 }">
                        <template v-if="item.variable.nMissing === 0">—</template>
                        <template v-else>{{ missingPct(item.variable).toFixed(0) }}%</template>
                    </span>
                </button>

                <Transition name="detail">
                    <div v-if="state.isExpanded(item.variable.name)" class="detail">
                        <ContinuousDetail v-if="isContinuous(item.variable)" :variable="item.variable" />
                        <IdDetail v-else-if="isId(item.variable)" :variable="item.variable" />
...
.list__item {
    ...
    transition: margin var(--dur-base) var(--ease-snap),
...
.row__type-dot.is-continuous { background: var(--accent); }
.row__type-dot.is-nominal    { background: #C28A4A; }
.row__type-dot.is-ordinal    { background: #6B9F6B; }
.row__type-dot.is-id         { background: var(--ink-4); }
